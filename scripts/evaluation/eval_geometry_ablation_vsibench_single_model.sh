...
#
# Usage:
#   bash scripts/evaluation/eval_geometry_ablation_vsibench_single_model.sh /path/to/model
#
# Optional env vars:
#   NUM_GPUS=8
#   EVAL_OUTPUT_ROOT=logs/eval_geometry_ablation_vsibench_single_model
...
DEFAULT_MODEL_PATH="/data7t-root/huggingface/hub/models--zd11024--vgllm-qa-vggt-8b/snapshots/73d51445fbb307c453c944bc3354aef68ec3cc59"
MODEL_PATH=${1:-${MODEL_PATH:-${DEFAULT_MODEL_PATH}}}
if [ -z "${MODEL_PATH}" ]; then
    echo "Usage: bash scripts/evaluation/eval_geometry_ablation_vsibench_single_model.sh <model_path>" >&2
    exit 1
fi
...
TASK_NAME=${TASK_NAME:-vsibench}
EVAL_OUTPUT_ROOT=${EVAL_OUTPUT_ROOT:-logs/eval_geometry_ablation_vsibench_single_model}
...
for exp_name in "${EXPERIMENTS[@]}"; do
    ...
    run_eval "${exp_name}" "${exp_output_dir}/${TASK_NAME}" "${log_file}"
...
echo "All single-model geometry ablation evaluations on ${TASK_NAME} finished."
