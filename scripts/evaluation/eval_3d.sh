...
# NOTE: This value is now *target ratio* w.r.t. loss_ce if auto-balance is enabled.
...
model_args_str="pretrained=$model_path,use_flash_attention_2=true,max_num_frames=32,max_length=12800,feature_fusion_method=$FEATURE_FUSION_METHOD,fusion_ortho_mode=$FUSION_ORTHO_MODE,fusion_lambda_ortho=1.0,fusion_lambda_auto_balance=$FUSION_LAMBDA_AUTO_BALANCE,fusion_lambda_target_ortho=$FUSION_LAMBDA_ORTHO,tune_mm_vision=$TUNE_MM_VISION,tune_mm_vision_lora=$TUNE_MM_VISION_LORA,tune_geometry_encoder=$TUNE_GEOMETRY_ENCODER,tune_geometry_encoder_lora=$TUNE_GEOMETRY_ENCODER_LORA,use_learnable_prefix=$USE_LEARNABLE_PREFIX,learnable_prefix_len=$LEARNABLE_PREFIX_LEN"
