...
class ModelArguments:
    ...
    text_gate_sentence_bert_name_or_path: Optional[str] = field(default=None)  # Frozen Sentence-BERT encoder for adver/adver_ortho gates; defaults to sentence-transformers/all-MiniLM-L6-v2 when unset
    text_gate_sentence_bert_max_length: Optional[int] = field(default=None)  # Maximum Sentence-BERT token length; defaults to 64 when unset
    text_gate_bert_name_or_path: Optional[str] = field(default=None)  # Legacy alias for text_gate_sentence_bert_name_or_path
    text_gate_bert_max_length: Optional[int] = field(default=None)  # Legacy alias for text_gate_sentence_bert_max_length
    ...
    label_weight_dynamic_iou_skip_invalid: bool = field(default=True)  # Skip dynamic IoU reweighting when predicted/target box text cannot be parsed reliably
    label_weight_loss_normalize_by_weight_sum: bool = field(default=True)  # If False, normalize weighted CE by valid token count so upweighted tokens increase total gradient budget
    ...
    fusion_gate_temperature: float = field(default=1.0)  # Temperature for the adver gate sigmoid; smaller values make the gate sharper
    fusion_gate_question_dim: int = field(default=16)  # Output dim of the gate-side linear projection applied to question_summary before fusion
    ...
    adver_freeze_llm_steps: int = field(default=150)  # In adver/adver_ortho with tune_mm_llm, freeze LLM (decoder + lm_head) for this many global steps; 0 disables
    gate_lr_decay_on_unfreeze: float = field(default=0.1)  # Multiply gate scheduler base_lr by this factor when LLM is unfrozen; 1.0 disables decay
...
class DataArguments:
    ...
    geometry_tokens: bool = field(default=False)  # Replace 3D bbox decimals with quantized geometry tokens for ScanRefer/ScanNetDet
    geometry_token_freeze_existing_rows: bool = field(default=True)  # Freeze gradients on original vocab rows so geometry-token training minimally disturbs existing tokens
...
class TrainingArguments(transformers.TrainingArguments):
    ...
    fusion_feature_3d_lr: Optional[float] = None
